...
namespace Runtime.Game.GameStates.Bootstrap.Controllers
{
    public class AudioSettingsBootstrapController : BaseController
    {
        ...
        {
            _audioService.SetVolume(AudioType.Sound, _userDataService.GetUserData().SettingsData.SoundVolume);
            _audioService.SetVolume(AudioType.Music, _userDataService.GetUserData().SettingsData.MusicVolume);
        }
    }
}